...
# url = "https://www.gardenia.net/plants/hardiness-zones"

# zones_req = requests.get(url, headers=headers)
# zones_req.raise_for_status()
# zones = BeautifulSoup(zones_req.text, 'html.parser')
# zones
# len(zones)
# zones.select('li a')

# zone_links = zones.select('.side-menu a')

# MA hardiness zones - 5, 6, 7
# MA_zones_links = zone_links[3:6]

# collect link
# zone_5_req = requests.get(USA_ZONE5_LINK, headers=headers)
# zone_5_page1 = BeautifulSoup(zone_5_req.text, 'html.parser')
...
# page1_plants_links = [link['href']
#                 for link in page1_links if link.text=="Read More"]
# len(page1_plants_links)  # 15

# collect id
# page1_plant_id = [link['data-set']
#                 for link in page1_links if link.text=="My Collection"]
# len(page1_plant_id)  # 15

# zone5_data = pd.DataFrame(list(zip(page1_plant_id, page1_plants_links)))
# zone5_data.columns = ["Id", "Link"]
